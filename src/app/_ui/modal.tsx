...
export default function Modal({ children, onOut, title }: PropsWithChildren<{ onOut?: Function, title?: string }>) {
    return (
        <div className="fixed h-screen w-full bg-gray-100 bg-opacity-50 z-50 top-0 left-0">
            <h1 onClick={() => onOut ? onOut() : ""} className="absolute m-5 rounded-lg z-10 bg-red-500 text-gray-50 font-bold w-max p-1 pr-3 pl-3 text-center cursor-pointer hover:bg-red-600 hover:text-red-50 transition-colors">x</h1>
